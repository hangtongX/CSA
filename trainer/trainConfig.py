...
class BaseTrainerConfig(BaseConfig):
    ...
    Parameters:
        ...
        optimizer_params (dict): A dict containing the params to use for the
            `torch.optim.Optimizer`. If None, uses the default params. Default: None.
        scheduler_cls (str): The name of the `torch.optim.lr_scheduler` used for
            training. If None, no scheduler is used. Default None.
        scheduler_params (dict): A dict containing the params to use for the
            `torch.optim.le_scheduler`. If None, uses the default params. Default: None.
    ...
    steps_predict: Union[int, None] = 10
    ...
    def _post_init(self):
        ...
        if self.optimizer_params is not None:
            try:
                ...
                raise TypeError(
                    "Error in optimizer's params. Check that the provided dict contains only "
                    f"keys and values suitable for `{optimizer_cls}` optimizer. "
                    f"Got {self.optimizer_params} as params.\n"
        ...
        if self.scheduler_cls is not None:
            ...
            if self.scheduler_params is not None:
                try:
                    ...
                    raise TypeError(
                        "Error in scheduler's params. Check that the provided dict contains only "
                        f"keys and values suitable for `{scheduder_cls}` scheduler. "
                        f"Got {self.scheduler_params} as params.\n"
                        f"Exception raised: {type(e)} with message: " + str(e)
                    ) from e
    ...
    def update(self, params: dict = None) -> dict:
        if params is not None:
            self.__dict__.update(params.train)
        else:
            params = munch.munchify(toml.load('./run.toml'))
            self.__dict__.update(params.train)
        self._post_init()
        return params
