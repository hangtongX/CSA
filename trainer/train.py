...
class BaseTrainer:
    ...
    def __init__(
        ...
        if self.training_config.output_dir is None:
            ...
            output_dir = os.path.join(output_dir, dataname, modelname)
        ...
        for epoch in range(1, self.training_config.num_epochs + 1):
            ...
            if (
                ...
                if best_model is None:
                    ...
                    pre_model = self.model
                rates = self.predict(pre_model, epoch)
    ...
    def train_step(self, epoch: int, **kwargs):
        ...
        for inputs in self.train_loader:
            ...
            if epoch_loss != epoch_loss:
                raise ArithmeticError("NaN detected in train loss", model_output)
    ...
    def load_checkpoint_model(self, dir_path: str, epoch: int = None):
        checkpoint_dir = os.path.join(dir_path, f"checkpoint_epoch/")
        state_dict = torch.load(os.path.join(checkpoint_dir, "model.pt"))
        self.model.load_state_dict(state_dict)
        self.model.eval()

    def predict(self, model: BaseModel = None, epoch: int = None):
        ...
        with self.amp_context:
            for inputs in self.test_loader:
                ...
                back = np.array(back).reshape(
                    len(self.evaluate.testmetric), len(self.evaluate.testmetrick), -1
                )
                if rates is None:
                    rates = back
                else:
                    rates = np.concatenate((rates, back), axis=2)
        ...
        return rates.transpose()

    def reinit(self, modelname=None, times=None, filelogger=None):
        ...
        if self.is_main_process:
            ...
            if log_output_dir is None:
                ...
                log_output_dir = os.path.join(
                    output_dir, self.dataname, self.model_name, 'logs'
                )
