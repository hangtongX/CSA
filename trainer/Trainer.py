...
class Trainer(object):
    def __init__(self, model, ranker, evaluator, optimizer,
                 loss_func, dataGenerater, config, DEVICE='cpu'):
        ...
        self.filename = '_'.join(
            [config.data_name, config.model_name, self.lossname])
    ...
    def train_one_epoch(self, epoch, start, early_stop=True):
        ...
        with tqdm(
            ...
            desc=set_color(
                f"{self.modelname},{self.lossname},{self.data_name},{epoch}",
                'green'),
            postfix=set_color(
                f'loss-{self.loss_result:.3f},val-{self.validate_str:.3f},gpu-{get_gpu_usage()},total time-{(time.time() - start) // 60} m {round((time.time() - start) % 60,0)} s',
                'cyan'),
            leave=True,
        ) as traindata:
        ...
        if epoch > 10 and early_stop:
            ...
            stop = self.evaluator.record_val(
                validate, epoch, self.model.state_dict())
            return stop
        else:
            return False
    ...
    def fit(self, earlystop=True, use_two_step=False):
                ...
                while self.model.get_rol() < 1e14:
                    ...
                    for _ in range(20):
                        self.train_one_epoch(
                            epoch=epoch, start=start, early_stop=True)
                        epoch += 1
        ...
        for epoch in range(epoch, self.epochs + epoch):
            stop = self.train_one_epoch(
                epoch=epoch, early_stop=True, start=start)
